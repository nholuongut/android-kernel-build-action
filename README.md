...
jobs:
  build-kernel:
    ...
    steps:
      - name: Build
        uses: nholuongut/android-kernel-build-action@main
        with:
...
```
Or use the [preset workflow file](https://github.com/nholuongut/android-kernel-build-action/blob/main/.github/workflows/build.yml) to modify it.
